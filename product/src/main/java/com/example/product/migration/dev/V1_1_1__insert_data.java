    ...
    public void migrate(Context context) throws Exception {
        try (Statement statement = context.getConnection().createStatement()) {
            ...
            List<ProductEntity> products = List.of(
                    new ProductEntity(null, "Product 1", BigDecimal.valueOf(10), 5, 1L, 10, categories.get(0), null, null),
                    new ProductEntity(null, "Product 2", BigDecimal.valueOf(20), 10, 2L, 20, categories.get(1), null, null),
                    new ProductEntity(null, "Product 3", BigDecimal.valueOf(30), 15, 1L, 30, categories.get(0), null, null)
            );
            products.forEach(product -> {
                String sql = String.format("INSERT INTO products (name, price, quantity, seller_id, stock, category_id) VALUES ('%s', %f, %d, %d, %d, %d)",
                        product.getName(), product.getPrice(), product.getQuantity(), product.getSellerId(), product.getStock(), product.getCategory().getId());
                try {
